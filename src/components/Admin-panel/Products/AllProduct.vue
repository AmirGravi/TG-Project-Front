<template>
    <h1 class="TextCssH">لیست محصولات</h1>
                    ...
                    <div class="col-md-4 mb-3" v-for="product in products" :key="product._id">
                        <div class="product-card">
                            ...
                            <div v-if="product.colors && product.colors.length" class="product-colors">
                                <p style="color: #ffa502;"><strong>رنگ‌ها : </strong></p>
                                <div class="color-swatches">
                                    <div v-for="color in product.colors" :key="color._id" class="color-item">
                                        <!-- نمایش رنگ‌ها با کد رنگ -->
                                        <span class="color-swatch" :style="{ backgroundColor: color.color }"></span>
                                        <!-- نمایش نام رنگ‌ها -->
                            ...
                            <div v-if="product.sizes && product.sizes.length" class="product-sizes">
                                <p style="color:#ff4757;"><strong>سایزها : </strong></p>
                                <div class="size-options">
                                    <span v-for="size in product.sizes" :key="size" class="size-option">{{ size
                                        }}</span>
                                </div>

                            </div>
                            <div class="mt-4">
                                <div>تعداد موجودی: <span class="quantityCss">{{ product.quantity }}</span> </div>
                            </div>
                        </div>
                    </div>
                </div>
...
<style scoped>
.box {
    box-shadow: 0 3px 20px #26de817d;
}

.TextCssH {
    box-shadow: 0 0px 8px #26de817d;
}
p{
   margin-bottom: 5px;
}
